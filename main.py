...
from data_loader import DropPercentDataset 
import torch
...
def train(model, train_loader, device, optimizer, criterion, epoch):
    ...
    for batch_idx, data in enumerate((train_loader)):
        # 初始化梯度为0
        optimizer.zero_grad()
        xs = data['xs_padded'].to(device).float()
        lens_xs_padded = data['lens_xs_padded']
        ys = data['ys'].to(device).float()
...
def split_data(train_folder, val_folder):
    def loader_collate(batch):
        xs = [torch.from_numpy(e['x']) for e in batch]
        x_lens = [e['x'].shape[0] for e in batch]
        ys = [torch.from_numpy(e['y']) for e in batch]
        ys = torch.stack(ys)
        xs = torch.nn.utils.rnn.pad_sequence(xs, batch_first=True)
        return {'xs_padded': xs, 'ys': ys, 'lens_xs_padded': x_lens}
    train_dataset = DropPercentDataset(train_folder)
    val_dataset = DropPercentDataset(val_folder)
...
def main():
    ...
    with open('./model_arch.txt', 'w+') as f:
        f.write(str(summary(model, input_size=(10, 20, 5), device='cpu')))
